    ...
    mocker.patch(
        ...
        return_value=ExecuteStatementResponse(
            ...
            status=StatementStatus(
                ...
                error=ServiceError(error_code=ServiceErrorCode.RESOURCE_EXHAUSTED, message="oops..."),
            ),
        ),
    )
...
def test_execute_poll_waits(config, mocker):
    ...
    def _get_statement(statement_id):
        ...
        if len(runs) == 0:
            runs.append(1)
            return GetStatementResponse(status=StatementStatus(state=StatementState.RUNNING), statement_id="bcd")
...
def test_execute_poll_timeouts_on_client(config, mocker):
    ...
    mocker.patch(
        "databricks.sdk.service.sql.StatementExecutionAPI.get_statement",
        return_value=GetStatementResponse(status=StatementStatus(state=StatementState.RUNNING), statement_id="bcd"),
    )
...
def test_fetch_all_no_chunks(config, mocker):
    ...
    rows = list(
        w.statement_execution.iterate_rows("abc", "SELECT id, CAST(NOW() AS DATE) AS since, NOW() AS now FROM range(2)")
    )
